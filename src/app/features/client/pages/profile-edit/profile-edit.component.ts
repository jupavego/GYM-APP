...
import { Membership, getPlanDef } from '../../../../core/models/membership.model';
...
export class ProfileEditComponent implements OnInit {
  ...
  private async loadMembership(): Promise<void> {
    this.membership.set(await this.membershipService.getMyMembership());
    this.membershipLoading.set(false);
  }

  get membershipActive(): boolean {
    return this.membershipService.isActive(this.membership());
  }

  get membershipPlanLabel(): string {
    const m = this.membership();
    return m ? getPlanDef(m.plan_id).label : '';
  }
